---
title: Hexo Butterfly优化合集-添加弹窗|修改加载动画|引入聊天机器人|css美化|修改链接悬停样式
tags: 教程
...
<div style="text-align: center;">
	<button id="post-button" onclick="show()">单击预览</button>
	<script>
	function show() {
...
在 `~\themes\butterfly\source\js\` 新建 `textWindow.js` 文件，复制下面的代码，**注意注释**
...
在 `~\themes\butterfly\source\css\` 新建 `textWindow.css` 文件，复制下面的代码，**注意注释**

```css
[data-theme='light'] #popup {/*根据网站明暗更改*/
    background-color: #e1e1e1;/*这里更改主体背景色*/
    box-shadow: 0 0 10px rgb(114, 113, 113);
  }
[data-theme='dark'] #popup {
    background: #3b443e ;/*这里更改主体背景色*/
...
#close {
    ...
    border-radius: 8px;
    z-index: 1000001;
    background-color: #b3b3b3;/*这里更改按钮背景色*/ 
  }
...
[动画来源](https://codepen.io/depy/pen/jOvymb)  你也可以以我的代码为模板，在这个网站挑选自己喜欢的动画当作加载动画。

1. 修改源代码

打开 `~\themes\butterfly\layout\includes\layout.pug` 按下面修改，大约在第十二行，**注意缩进**
...
2.  在 `~\themes\butterfly\layout\includes\loading\` 新建`loaded.ejs` ，复制下面的代码
...
3. 在 `~\themes\butterfly\source\css\` 新建`loaded.css` ，复制下面的代码
...
打开 `~\themes\butterfly\source\css\_layout\post.styl`  ，按下面修改，大约在80行
...
在 `~\themes\butterfly\source\js\` 新建 `sakura.js` 文件，复制下面的代码。
...
例如我只想在首页引入 `sakura.js` ，可以这样修改：**注意缩进**

```pug
if !theme.disable_top_img && page.top_img !== false
+  if is_home()
+    link(rel="stylesheet", href="/js/sakura.css")
```

如果你使用了pjax，会发现尽管仅在首页引入 `sakuea.js` 但是樱花特效每个页面都会出现，这时我们需要修改代码。首先把之前引入的 `sakura.js` 删除 **（不是删除文件，是删除引入）** ，同一目录新建另外一个 `better_sakura.js` 文件，插入以下代码。

这里还加了一个移动设备判断，如果不需要把那句判断删去即可 **（大括号也要删干净）**

```javascript
// 监听 pjax:complete 事件
document.addEventListener('pjax:complete', function () {
    if (!/Mobi|Android|iPhone/i.test(navigator.userAgent)) {
        if (window.location.pathname === '/') {//这里改成你想插入的页面的地址，例如首页是'/'，友链页面是'/link/'
...
```javascript
    SakuraList.prototype.draw = function (cxt) {//如果不是主页就返回
+        if (window.location.pathname !== '/') {//这里要填你想添加特效的网页相对位置。
+            return;//例如想添加到友链界面，需要写window.location.pathname !== '/link/'
+        }
        for (var i = 0, len = this.list.length; i < len; i++) {
            this.list[i].draw(cxt);
        }
...
我在bing站长工具扫描网站时，发现了一个问题，如图

网站的每个页面都有一张图片缺少了alt属性，借助bing url检查的帮助，我发现是网站的图标缺少了alt属性。
...
> 如果是通过npm安装，
>
> 地址为``./node_modules/hexo-theme-butterfly/layout/includes/header/nav.pug``
>
> 如果是从github clone下来的，
>
> 地址是``./themes/butterfly/layout/includes/header/nav.pug``

```html
img.site-icon(src=url_for(theme.nav.logo), alt="Site Icon")
#引号内的Site Icon可以替换
```
